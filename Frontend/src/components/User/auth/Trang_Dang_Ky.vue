						...
						<div class="w-100" style="max-width: 550px;">
							...
							<div class="d-flex align-items-center justify-content-between mb-5 px-3 position-relative">
								...
								<div class="step-badge-wrapper text-center z-1">
									<div class="step-badge rounded-circle d-flex align-items-center justify-content-center mx-auto mb-1 transition-all"
										:class="currentStep >= 1 ? 'bg-primary text-white shadow-primary' : 'bg-light text-muted border'">
										1
									</div>
								...
								<div class="step-badge-wrapper text-center z-1">
									<div class="step-badge rounded-circle d-flex align-items-center justify-content-center mx-auto mb-1 transition-all"
										:class="currentStep >= 2 ? 'bg-primary text-white shadow-primary' : 'bg-light text-muted border'">
										2
									</div>
								...
								<div class="step-badge-wrapper text-center z-1">
									<div class="step-badge rounded-circle d-flex align-items-center justify-content-center mx-auto mb-1 transition-all"
										:class="currentStep >= 3 ? 'bg-primary text-white shadow-primary' : 'bg-light text-muted border'">
										3
									</div>
							...
							<form @submit.prevent="handleRegister" class="needs-validation">
								...
								<div v-show="currentStep === 1" class="animation-fade-in">
									...
									<div class="row g-3">
										<div class="col-12">
											...
											<input type="text" class="form-control bg-light border-0 py-2 px-3 input-focus" id="ho_ten" v-model="form.ho_ten" :placeholder="$t('auth.register.fullNamePlaceholder')" required>
										</div>
										<div class="col-12">
											<label for="email" class="form-label fw-semibold text-dark">{{ $t('auth.common.emailLabel') }}</label>
											<input type="email" class="form-control bg-light border-0 py-2 px-3 input-focus" id="email" v-model="form.email" placeholder="example@email.com" required>
										</div>
										<div class="col-12">
											<label for="phone" class="form-label fw-semibold text-dark">{{ $t('auth.common.phoneLabel') }}</label>
											<input type="tel" class="form-control bg-light border-0 py-2 px-3 input-focus" id="phone" v-model="form.so_dien_thoai" :placeholder="$t('auth.register.phonePlaceholder')">
										</div>
										<div class="col-12">
											<label for="pw1" class="form-label fw-semibold text-dark">{{ $t('auth.common.passwordLabel') }}</label>
											<div class="input-group">
												<input :type="showPassword ? 'text' : 'password'" class="form-control bg-light border-start-0 border-top-0 border-bottom-0 border-end-0 py-2 px-3 input-focus border-light" id="pw1" v-model="form.password" :placeholder="$t('auth.register.pwPlaceholder')" required style="border-radius: 8px 0 0 8px;">
										...
										<div class="col-12">
											...
											<input type="password" class="form-control bg-light border-0 py-2 px-3 input-focus" v-model="form.password_confirmation" :placeholder="$t('auth.register.confirmPwPlaceholder')" required>
										</div>
								...
								<div v-show="currentStep === 2" class="animation-fade-in">
									<div class="mb-4">
										...
										<div v-else class="d-flex flex-wrap gap-2 mt-3">
											<span v-for="skill in availableSkills" :key="skill.id"
												class="badge rounded-pill px-3 py-2 cursor-pointer transition-hover user-select-none"
												:class="form.ky_nang_ids.includes(skill.id) ? 'bg-primary text-white shadow-sm' : 'bg-light text-dark text-opacity-75'"
												style="font-size: 14px; border: 1px solid var(--bs-gray-200);"
									...
									<div class="mb-4">
										...
										<div class="d-flex flex-wrap gap-2">
											<span v-for="area in availableAreas" :key="area.id"
												class="badge rounded-pill px-3 py-2 cursor-pointer transition-hover user-select-none"
												:class="form.khu_vuc_ids.includes(area.id) ? 'bg-success text-white shadow-sm' : 'bg-light text-dark text-opacity-75'"
												style="font-size: 14px; border: 1px solid var(--bs-gray-200);"
								...
								<div v-show="currentStep === 3" class="animation-fade-in">
									<div class="text-center mb-4">
										<div class="bg-success bg-opacity-10 text-success d-inline-flex align-items-center justify-content-center rounded-circle mb-3"
										...
										<div class="row mb-3 pb-3 border-bottom border-light-subtle">
											...
											<div class="col-12">
												<div class="d-flex flex-wrap gap-1">
													...
													<span v-if="form.khu_vuc_ids.length === 0" class="badge bg-secondary opacity-50">Chưa chọn</span>
												</div>
											</div>
										</div>
										<div class="row">
											...
											<div class="col-12">
												<div class="d-flex flex-wrap gap-1">
													...
													<span v-if="form.ky_nang_ids.length === 0" class="badge bg-secondary opacity-50">{{ $t('auth.register.noSkills') }}</span>
												</div>
											</div>
										</div>
									</div>
							...
							<div class="mb-4 form-check user-select-none">
								<input type="checkbox" class="form-check-input shadow-none cursor-pointer" id="agreeTerms" v-model="form.agreeTerms" required>
...
export default {
	...
	data() {
		return {
			...
			form: {
				...
				ky_nang_ids: [], khu_vuc_ids: [], agreeTerms: false
			},
			availableSkills: [],
			availableAreas: [],
		}
	...
	methods: {
		...
		getSkillName(id) {
			const s = this.availableSkills.find(s => s.id === id);
			return s ? s.ten : '';
		},
		getAreaName(id) {
			const a = this.availableAreas.find(a => a.id === id);
			return a ? a.ten : '';
		},
...
/* Scrollbar customization for right panel */
